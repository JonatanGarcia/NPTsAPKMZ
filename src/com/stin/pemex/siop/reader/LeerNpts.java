 ...
 */
package com.stin.pemex.siop.reader;
...
public class LeerNpts {
    ...
    public List<Npt> leerExcelNPT(String archivoDestino, int idPozo, int intervencion, int tipoIntervencion, boolean existe, String interBuscar, boolean visualiza, int ultimoConse) {
...
//                            }//           
                    ...
                    for (int fila = filaBuscar; fila < numFilas; fila++) { // Recorre cada fila de la hoja                        
                        ...
                        if (veces > 0 && visualiza) {
                            if (sumaHoras < 24) {
                                ...
                                JOptionPane.showMessageDialog(null, "Existe NPTS calificados mayor a 24 horas \nEn el Consecutivo: " + sheet.getRow(fila - 1).getCell(1).toString().replace(".0", ""), Menu.nombreSistema, JOptionPane.ERROR_MESSAGE);
                                throw new Exception("");
                            }
                        }
    ...
    public static List<Programas> getProgramaPerforacion(String excelName, int idIntervencion) {
        ...
        try {
            ...
            String oldEtapa="", oldTr="";
            double ultimoConsecutivo=0;
            double etapaConse=0;
            boolean asignaOld=true;
            ...
            for (int fila = filaBuscar; fila < sheet.getLastRowNum(); fila++) {
                ...
                if (asignaOld){
                    oldEtapa=sheet.getRow(fila).getCell(4).toString();
                    oldTr=sheet.getRow(fila).getCell(5).toString();
                    ultimoConsecutivo=sheet.getRow(fila).getCell(11).getNumericCellValue();
                    asignaOld=false;
                }
                if(oldEtapa ==sheet.getRow(fila).getCell(4).toString() && oldTr==sheet.getRow(fila).getCell(5).toString()){
                    double consecutivoActual=sheet.getRow(fila).getCell(11).getNumericCellValue();
                    for(double i=ultimoConsecutivo;i<=consecutivoActual;i++){
                        /*listaPrograma.add(new Programas(
                            ...
                            i));//sheet.getRow(fila).getCell(11).getNumericCellValue()));
                    */
                        
                     System.out.println("New consecutivo: " + i + " etapaConse" + etapaConse +" etapa "+ sheet.getRow(fila).getCell(4).toString() +
                             " tr " + sheet.getRow(fila).getCell(5).toString() + " prof: " + sheet.getRow(fila).getCell(6).getNumericCellValue());
                    }
                }else{
                    oldEtapa=sheet.getRow(fila).getCell(4).toString();
                    oldTr=sheet.getRow(fila).getCell(5).toString();
                    ...
                    ultimoConsecutivo=sheet.getRow(fila).getCell(11).getNumericCellValue();
                    etapaConse=sheet.getRow(fila-1).getCell(11).getNumericCellValue();
                    System.out.println(etapaConse+ " , "+ ultimoConsecutivo);
                }
            }
        error=4;
        
//        for( Programas p: listaPrograma){
//            System.out.println("Etapa: " + p.getCatEtapa().getStrNombre() + " Tr " +p.getCatTr().getStrNombre() + " tiempo "+p.getFloatTiempo()+" Profundidad "+p.getIntProf());
//        }
        } catch (Exception e) {
    ...
    public ArrayList leerArchivo(String archivo) {
        ...
        ArrayList listaNpts = new ArrayList();
        ...
        try {
            ...
            while ((linea = br.readLine()) != null) {
                Npt myNpt = new Npt();
                String[] elementosNpt = linea.split(",");
                myNpt.setPozo(Integer.valueOf(elementosNpt[0]));
                myNpt.setTiempo(Float.valueOf(elementosNpt[1]));
                myNpt.setFecha(elementosNpt[2]);
                //myNpt.setIdNpt(Integer.valueOf(elementosNpt[3]));
                listaNpts.add(myNpt);
                //contenido += linea + "\n";
            }
            ...
            try {
                ...
                System.out.println("NptsReader.leerArchivo: " + ex.getMessage());
            }
        }
        return listaNpts;
    }

    public void imprimeNpts(List listaNpts) {
        //Imprime todos los npts del pozo.
        try {
            for (int j = 0; j < listaNpts.size(); j++) {
                System.out.println("============================================================");
                System.out.println("NPT numero " + j);
                Npt myNpt = (Npt) listaNpts.get(j);
                System.out.println("Pozo: " + myNpt.getPozo());
                System.out.println("Intervencion: " + myNpt.getIntervencion());
                System.out.println("Consecutivo: " + myNpt.getConsecutivo());
                System.out.println("Fecha: " + myNpt.getFecha());
                System.out.println("Profundidad: " + myNpt.getProfundidad());
                System.out.println("Resumen: " + myNpt.getResumen());
                System.out.println("Tiempo: " + myNpt.getTiempo());
                System.out.println("============================================================");
            }

        } catch (Exception e) {
            System.out.println("imprimeNpts: " + e.getMessage());
        }
